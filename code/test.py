...
'''
def capsule():
    if not routing:         #first capsule
        capsule = []        #create capsule array
        for i in range(length_output):
            caps_i = covolve(input, weights)
            caps_i = reshape(conv_i, (batch_size,-1,1,1))
            capsule.append[caps_i]

        capsule = concat(capsule, axis=2)
        
        vec_abs = square(capsule)
        vec_abs = reduce_sum(vec_abs)
        vec_abs = sqrt(vec_abs)

        scalar_fator = square(vec_abs)
        scalar_factor = scalar_factor / (1+scalar_factor)

        capsule = scalar_factor*divide(capsule, vec_abs)

    else:                   #subsequent capsules
        B_ij = zeros([1, caps1.shape[1], caps1.shape[2]+1, 1], type)
        capsule = []       #create new capsule
        
        for j in range(number_outputs):
            W_initializer = random([1, caps1.shape[1], caps1.shape[2], 16]) # digit caps weight
            W_ij = Variable(W_initializer)
            W_ij = tile(W_ij, [batch_size, 1,1,1])
            u_hat = matmul(W_ij, input)
            for r in range(routing_iterations):
                C_ij = softmax(B_ij, 2)
                B_il, B_ij, B_ir = split(B_ij, [j, 1, B_ij.shape[2]-j-1], axis=2)
                C_il, C_ij, B_ir = split(C_ij, [j, 1, B_ij.shape[2]-j-1], axis =2)

                S_j = multiply(C_ij, u_hat)
                S_j = reduce_sum(C_ij, axis = 1)
                ...
                V_j = scalar_factor*divide(S_j, vec_abs)

                V_j_tiled = tile(V_j, [1, caps1.shape[1], 1, 1])
                U_produce_V = matmul(u_hat, V_j_tiled, transpose_a=True) 
                B_ij += reduce_sum(U_produce_V, axis = 0, keep_dims = True)
                B_ij = concat([B_il, B_ij, B_ir], axis = 2)
            capsule.append(V_j)
        capsule = concat(capsule, axis=1)
